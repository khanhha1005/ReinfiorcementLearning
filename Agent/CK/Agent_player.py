...
def player_Matran_Random1(play_state,file_temp,file_per):
    a = get_list_action(play_state)
    if len(file_temp) < 2:
        file_temp = [(np.random.random((len(play_state),100)))*2 -0.6,np.random.random((100,amount_action_space()))]

    Result_matran1 = np.matmul(play_state, file_temp[0])
    Result_matran1 = silu(Result_matran1, theda = 1.0)
...
# Buoc 2:
def player_Matran_Score1(play_state,file_temp,file_per):
    a = get_list_action(play_state)

    if len(file_per) < 2:
        file_ = np.load('CK_Matran.npy',allow_pickle=True)
...
def test(play_state,file_temp,file_per):
    ...
    if len(file_temp) < 2:
        file_temp = np.load('CK_Win.npy',allow_pickle=True)
...
# number_tran la so nguyen duong >=1, neu muon chay X0.000 tran thi hay dien number_tran = x
    # Buoc1:
    list_player= [player_Matran_Random1,player_Matran_Random1,player_Matran_Random1,player_random]
    kq, file_ = normal_main(list_player, 1500, [0])
    np.save(f'{path_save_player}CK_Matran.npy',file_)
    # Buoc2:
    list_player= [player_Matran_Score1,player_Matran_Score1,player_Matran_Score1,player_random]
    ...
    for buoc3 in range(2*number_tran-1):
        list_player = [player_Matran_Random1,player_Matran_Random1,test1,player_random]
        kq, file_ = normal_main(list_player,1500, [0])
